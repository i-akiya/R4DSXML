getDLMD <- function(filepath) {
    ...
    namespaces <- namespaces(doc)
        ...
        IGD_IsReferenceData <-
            getAttr(Nodeset = ItemGroupDef, Attr = "IsReferenceData")
        IGD_SASDatasetName <-
            getAttr(Nodeset = ItemGroupDef, Attr = "SASDatasetName")
        IGD_Purpose <- getAttr(Nodeset = ItemGroupDef, Attr = "Purpose")
        IGD_Structure <- getAttr(Nodeset = ItemGroupDef, Attr = "Structure")
        # for Define-XML v2.0
        if(namespaces[["def"]] == "http://www.cdisc.org/ns/def/v2.0") {
        ...
        IGD_Description <-
            sapply(IGD, function(el)
                xmlValue(
                    el,
                    'ns:Description/ns:TranslatedText[@xml:lang = "en"]'
                ))
        IGD_title <-
            sapply(IGD, function(el)
